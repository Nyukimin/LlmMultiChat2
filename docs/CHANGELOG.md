...
- Ingest Mode（DB登録専用モード）を追加（JSON抽出→自動登録、CLI）
  - 検索先行フローへ変更（DuckDuckGoの要約を同梱して1回呼び）
  - 抽出スキーマに `next_queries`（最大5件、日本語、重複除去）を追加
  - 厳格JSON化のため抽出ロジックを強化（マーカー優先、コードフェンス抽出、最外括弧フォールバック）
  - 不具合修正: `build_extractor_prompt()` が `return` を返さず `None` となる問題を修正
